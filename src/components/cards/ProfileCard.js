...
const Input = tw.input`mt-1 first:mt-0 border-b-2 py-3 focus:outline-none font-medium transition duration-300 hocus:border-pink-700`
const RadioInput = tw.input`mt-6 first:mt-0 border-b-2 py-3 focus:outline-none font-medium transition duration-300 hocus:border-pink-700`
const Label = tw.label`mt-6 first:mt-0 py-0 focus:outline-none text-sm font-medium transition duration-300 hocus:border-pink-200 text-start`
const Select = tw.select`mt-1 first:mt-0 border-b-2 py-3 focus:outline-none font-medium transition duration-300 hocus:border-pink-700`
...
const CardContent = styled.div`
  ...
  .position {
    ${tw`uppercase font-bold tracking-widest text-xs text-pink-700`}
  }
  .bio {
...
const CardLinks = styled.div`
  ${tw`mt-6 flex`}
  .link {
    ${tw`mr-8 last:mr-0 text-gray-400 hocus:text-pink-700 transition duration-300`}
    .icon {
...
export default ({
      ...
      .then(async response => {
        ...
        if (!response.ok) {
          if (response.status === 400) {
            ...
            if (data.message) setError(data.message || templateErrorMsg)
          } else {
  ...
  }, [setUserContext, uid, userContext.token])
        ...
        <TwoColumn>
          <Cards>
            <Card>
              ...
              <CardContent>
                <span className="position">{role === "user" ? "Customer" : (role === "admin"? "admin" : "Owner")}</span>                <span className="name">{firstName + " " + lastName}</span>
